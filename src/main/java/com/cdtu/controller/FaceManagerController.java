...
public class FaceManagerController {
	...
	@RequestMapping("/add")
	@ResponseBody
	public String addFace(@RequestBody Map<String,Object> paramMaps,HttpServletRequest request, HttpServletResponse response){
...
//		if(paramMaps.get("username").equals("")||null==paramMaps.get("username")){
...
//			String userId = UUID.randomUUID().toString().replace("-", "").toUpperCase();//用户id（由数字、字母、下划线组成），长度限制128B
			...
			org.json.JSONObject resultObject = aipFace.addUser(imgdata, "BASE64", groupId, userId, options);
