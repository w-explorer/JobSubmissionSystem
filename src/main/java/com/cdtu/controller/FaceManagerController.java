...
public class FaceManagerController {
	...
	@RequestMapping("/add")
	@ResponseBody
	public Map<String, Object> addFace(@RequestBody Map<String, Object> paramMaps, HttpServletRequest request,
		...
		if ("0".equals(faceSerachResponse.getError_code()) && "SUCCESS".equals(faceSerachResponse.getError_msg())) {
			map.put("status", 200);
			map.put("msg", "注册成功！");
		} else if ("0".equals(faceSerachResponse.getError_code())
				&& "face is already exist".equals(faceSerachResponse.getError_msg())) {
			map.put("status", 222);
			map.put("msg", "人脸信息已存在");
		} else {
			map.put("status", 500);
	...
	@ResponseBody
	public Map<String, Object> searchFace(@RequestBody FacePageBean facePageBean, HttpServletRequest request,
			HttpServletResponse response) {
		...
		if ("0".equals(faceSerachResponse.getError_code()) && "SUCCESS".equals(faceSerachResponse.getError_msg())) {
			if (faceSerachResponse.getResult().getUser_list().get(0).getScore() > 80f) {
				...
				// 构造一个用户名密码令牌 ,是否记住我
				UsernamePasswordToken token1 = new UsernamePasswordToken(userName, password, roleName);
				try {
